...
export default function SubmoduleContainer( props: { children: any; } ) {
    ...
    <div className="flex flex-col flex-grow items-center justify-between p-4 border border-gray-200 rounded-lg shadow-sm">
        <h2 className="text-xl font-bold mb-4">{submoduleTitleList[currentModuleIndex]}</h2>
        <div className="flex-1">{children}</div>
        <div className="flex justify-between w-full mt-4">
        <Button color="gray" onClick={() => routerPush(currentModuleIndex - 1)} disabled={currentModuleIndex - 1 < 0}>
            Prev
        </Button>
        <Button color="gray" onClick={() => routerPush(currentModuleIndex + 1)} disabled={currentModuleIndex + 1 >= submodulePathOrdering.length}>
            Next
        </Button>
        </div>
    </div>
