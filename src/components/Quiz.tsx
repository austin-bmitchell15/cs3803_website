...
export default function QuizComponent({
  ...
  const [selectedOption, setSelectedOption] = useState(
    Array.from({ length: quiz.length }, () => -1),
  );
    ...
    <div>
      <div className="flex mt-7 w-full">
        <div className="w-1/3">
            ...
            {quiz.map((q, i) => {
              return (
                <ListGroup.Item
                  ...
                  className={
                    checkedMode
                      ? selectedOption[i] == q.answerOption
                        ? "text-green-700"
                        : "text-red-500"
                      : ""
                  }
                  active={i == viewedQuestionIndex}
                  onClick={() => setViewedQuestionIndex(i)}
                >
                  Question {i}
                </ListGroup.Item>
              );
          ...
          {checkedMode ? (
            <Button className="mt-4" color="warning" onClick={retakeQuiz}>
              Redo
            </Button>
          ) : (
            <Button
        ...
        <div className="w-2/3 ml-10 mr-5">
          {!checkedMode ? (
            <QuestionView
              question={quiz[viewedQuestionIndex]}
              selectOption={(option: number) =>
                updateIndex(viewedQuestionIndex, option)
              }
              selectedOption={selectedOption[viewedQuestionIndex]}
            />
          ) : (
            <SubmittedQuestionView
              question={quiz[viewedQuestionIndex]}
              selectedAnswer={selectedOption[viewedQuestionIndex]}
            />
          )}
        </div>
      </div>
...
function ModalComponent({
  ...
  for (var i = 0; i < optionsAnswered.length; i++) {
    if (optionsAnswered[i] == correctAnswers[i]) n_correct += 1;
  }
  ...
  return (
    <Modal show={true} onClose={() => setShowModal(false)}>
      ...
      <Modal.Footer>
        <Button color={passed ? "light" : "failure"} onClick={retakeQuiz}>
          Try Again
        </Button>
...
function SubmittedQuestionView({
  question,
  selectedAnswer,
}: {
  question: QnAType;
  ...
  return (
    <Card className="max-w-5xl max-h-sm max-h-90 overflow-auto p-4">
      ...
      <p className="px-2 text-gray-600">{question.explanation}</p>
      <div>
        <fieldset className="flex max-w-md flex-col gap-4">
          {/* <legend className="mb-4">Choose your favorite country</legend> */}
          {question.options.map((opt, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <Label className={textColors[i]} htmlFor={opt}>
                {opt}
              </Label>
            </div>
          ))}
